...
exports.Intel8080 = exports.Flag = void 0;
var Flag;
(function (Flag) {
    /** Sign flag. */
    Flag[Flag["S"] = 7] = "S";
...
class Intel8080 {
    ...
    applyAddFlags(accumulatorValue, value, carryIn = 0) {
        ...
        this.setFlag(Flag.A, (accumulatorValue & 0x0f) + (value & 0x0f) + carryIn > 0x0f);
        return result8Bit;
    }
    applySubFlags(accumulatorValue, value, borrowIn = 0) {
        const result = accumulatorValue - value - borrowIn;
        const result8Bit = result & 0xff;
        this.updateZeroSignParityFlags(result8Bit);
        this.setFlag(Flag.C, accumulatorValue < value + borrowIn);
        this.setFlag(Flag.A, (accumulatorValue & 0x0f) - (value & 0x0f) - borrowIn < 0);
        return result8Bit;
    }
    applyLogicalFlags(result, auxiliaryCarry) {
        const result8Bit = result & 0xff;
        this.updateZeroSignParityFlags(result8Bit);
    ...
    evaluateCondition(conditionCode) {
    ...
    writeRegisterOrMemory(registerCode, value) {
        const registerName = this.getRegisterNameByCode(registerCode);
        const value8Bit = value & 0xff;
        if (registerName === 'M') {
    ...
    setRegisterPair(pairName, value) {
    ...
    executeTransferInstruction(opcode) {
    ...
    executeAluInstruction(opcode) {
        ...
        if (opcode === 0xce) {
            const immediateValue = this.getNextByte();
            const carryIn = this.isFlagSet(Flag.C) ? 1 : 0;
            this.registers.A = this.applyAddFlags(this.registers.A, immediateValue, carryIn);
            return {
        ...
        if (opcode === 0xde) {
            const immediateValue = this.getNextByte();
            const borrowIn = this.isFlagSet(Flag.C) ? 1 : 0;
            this.registers.A = this.applySubFlags(this.registers.A, immediateValue, borrowIn);
            return {
        ...
        if (opcode === 0xe6) {
            const immediateValue = this.getNextByte();
            this.registers.A = this.applyLogicalFlags(this.registers.A & immediateValue, true);
            return {
        ...
        if (opcode === 0xee) {
            const immediateValue = this.getNextByte();
            this.registers.A = this.applyLogicalFlags(this.registers.A ^ immediateValue, false);
            return {
    ...
    executeControlFlowInstruction(opcode) {
        if (opcode === 0xc3) {
    ...
    executeStackInstruction(opcode) {
        const pushPairs = ['B', 'D', 'H', 'PSW'];
        if ((opcode & 0xcf) === 0xc5) {
            const pairIndex = (opcode >> 4) & 0x03;
            const pairName = pushPairs[pairIndex];
            if (pairName) {
                const pairValue = pairName === 'PSW'
    ...
    executeSubroutineInstruction(opcode) {
        if (opcode === 0xcd) {
    ...
    executeMemoryAndPairInstruction(opcode) {
        if ((opcode & 0xcf) === 0x03) {
            ...
            const currentValue = pairName === 'SP'
        ...
        if (opcode === 0x0a) {
            const address = ((this.registers.B & 0xff) << 8) | (this.registers.C & 0xff);
            this.registers.A = this.bus.readRam(address);
            return {
                Disassemble: 'LDAX B',
                Ticks: 7
        ...
        if (opcode === 0x1a) {
            const address = ((this.registers.D & 0xff) << 8) | (this.registers.E & 0xff);
            this.registers.A = this.bus.readRam(address);
            return {
                Disassemble: 'LDAX D',
                Ticks: 7
    ...
    in(port) {
        ...
        this.log(`Received ${String.fromCharCode(this.registers.A)} from device #${port}`);
        return {
